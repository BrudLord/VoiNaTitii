...
function enchantmentSummary(item){return (item.data.enchantments||[]).map(id=>{const e=(S.rules.enchantments||[]).find(x=>x.id===id);return e?`<details class="item-enchantment"><summary>${esc(e.name)}</summary><p>${esc(e.description)}</p></details>`:''}).join('')+(item.data.upgrades||[]).map(id=>{const e=entry(id);return e?`<details class="item-enchantment"><summary>${esc(e.name)}</summary><p>${esc(e.description)}</p></details>`:''}).join('')}
