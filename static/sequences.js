...
function sequenceForm(a){
 ...
 modal(a.name,`<div class="entry-desc">${esc(a.description)}</div><p>${esc(spec.targets==='same'?'Все удары по одной цели':spec.targets==='each'?count+' атаки по каждому получателю':spec.targets==='different'?'Каждая атака по разной цели':'Выберите цель каждого удара')}</p>${spec.minimum?input('series_count','Количество атак',count,'number',`min="${spec.minimum}" max="${count}" step="${spec.attacks_per_step||1}" required`):''}<div id="series-targets" class="fields" data-groups="${groups}">${sequenceTargets(groups)}</div>${spec.targets==='each'?btn('+ Получатель','series.add')+btn('− Получатель','series.remove','title="Убрать последнего получателя"'):''}${a.use_ready?readyFields(c,a.ready_id):''}`,async()=>{
  ...
  const root={op:'ability.use',character:c.id,ability:a.id,sequence_revision:S.revision,as_reaction:!!a.as_reaction,support_minor:!!a.support_minor,...readyPayload(a,f),...(a._draw_weapon?{draw_weapon:a._draw_weapon}:{}),...(spec.one_per_step?{steps:n/(spec.attacks_per_step||1)}:{})};
...
function sequenceReload(a,c){
 if(!a.data.weapon||!c.calc.needs_reload)return '';
 const action=c.calc.reload_action,label={main:'основное',minor:'малое'}[action]||action;
...
function sequenceMovement(a){
 const s=sequenceCapture?.ability.attack_sequence||a.attack_sequence,c=current();if(s.one_per_step&&sequenceCapture.index%(s.attacks_per_step||1))return '';const directions=s.during_movement?['before','after']:s.one_per_step?['before']:s.step_after_attack?['after']:[];
