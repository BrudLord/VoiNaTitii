...
function craftFields(spec,stock){
 if(!spec)return '';
 const materials=`<details><summary>Материалы</summary><div class="fields">${stock.filter(i=>!i.equipped).map(i=>input('supply'+i.id,i.label,0,'number',`min="0" max="${i.quantity}"`)).join('')}</div></details>`;
 if(spec.kind==='production')return `<p class="description">${esc(entry(spec.id)?.description)}</p><p>Один расходник · ${spec.hours} часа приготовления</p>${materials}<label class="checks"><input type="checkbox" name="prepared">Приготовление в игровом мире завершено</label>`;
...
function craftingForm(c,recipeId=null){
 ...
 if(!recipes.length){modal('Мастерская','<p>Выберите ремесло в листе персонажа и изучите рецепт в разделе знаний.</p>',null);return}
 const stock=craftStock(c),selected=recipes.find(r=>r.id===recipeId)||recipes[0];
 modal('Мастерская',`${selector('craft_recipe','Рецепт',recipes,selected.id)}<div id="craft-fields">${craftFields(selected,stock)}</div>`,async()=>{
  const f=formObject(),spec=recipes.find(r=>r.id===num(f.craft_recipe)),item=stock.find(i=>i.id===num(f.craft_item));
  if(!spec||spec.kind!=='production'&&!item)throw Error('Выберите рецепт и предмет');
  ...
  await api({op:'craft.apply',character:c.id,recipe:spec.id,item:item?.id,item_revision:item?.revision,prepared:!!f.prepared,supplies});
 },'Улучшить предмет');
 const update=()=>{
  const f=formObject(),spec=recipes.find(r=>r.id===num(f.craft_recipe));let valid=spec?.kind==='production'?!!f.prepared:!!num(f.craft_item);
  el('dialog-submit').textContent=spec?.kind==='production'?'Приготовить':'Улучшить предмет';
  if(spec?.kind==='enchantment'){
...
document.addEventListener('click',e=>{const b=e.target.closest('[data-do="craft.open"]');if(b)craftingForm(current(),num(b.dataset.recipe))});
