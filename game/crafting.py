...
def apply(user,p):
    ...
    def stock(pk,revision):
        ...
        return item
    target=stock(p['item'],p.get('item_revision'))
    if target.quantity<1:raise ValueError('Нет предмета для улучшения')
    field='enchantments' if spec['kind']=='enchantment' else 'upgrades'
    if recipe.id in target.data.get(field,[]):raise ValueError('Это улучшение уже есть на предмете')
    ...
    elif not weaponry.compatible(target,spec):raise ValueError('Улучшение не подходит к этому оружию')
    rows=p.get('supplies',[])
    ...
    result.data.setdefault(field,[]).append(recipe.id)
