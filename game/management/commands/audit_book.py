...
class Command(BaseCommand):
    ...
    def handle(self, **options):
        ...
        for kind,name,desc,data in records(text):
            entry,created=Entry.objects.get_or_create(kind=kind,name=name,defaults={'description':desc or '', 'data':data,'source':'Книга игрока'})
            if not created:
        ...
        for name,desc,data,source in abilities(text):
            ...
            entry=Entry.objects.filter(kind='ability',source=source).first()
            if not entry: entry=Entry.objects.filter(kind='ability',name=display).first()
        ...
        for name in ['Живучесть','Укрепленные органы']:
            e=Entry.objects.filter(kind='ability',name=name).first()
            if e and not e.data.get('reviewed'):
                e.data['passive_hp']=10
                e.save(update_fields=['data'])
        for e in Entry.objects.filter(kind='ability',name__in=['Стандартная атака','Провоцированная атака']):
            e.data['system']=True
            for key,value in dict(weapon=True,damage=True,rolls=True,formula='1Ор + Мод').items():
                e.data.setdefault(key,value)
            e.save(update_fields=['data'])
        Entry.objects.get_or_create(kind='ability',name='Провоцированная атака',defaults={
        ...
        Entry.objects.filter(kind='specialization',name__in=['Географические','Профессии','Стихии','Навыки','Местности']).update(archived=True)
        ...
        for line in section.splitlines():
            ...
            e=Entry.objects.filter(kind='item',name=name).first()
            ...
            e.save(update_fields=['data'])
        for word in sorted({k for e in Entry.objects.filter(kind='ability') for k in e.data.get('keywords',[]) if len(k)<160}):
            Entry.objects.get_or_create(kind='keyword',name=word,defaults={'source':'Книга игрока'})
