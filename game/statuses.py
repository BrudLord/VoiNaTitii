...
STATUS = {'Поджог':('status',1),'Влага':('status',1),'Кислота':('ac',-1),'Шок':('status',1),
          ...
          'Обездвижен':('status',1),'Ослепление':('status',1),'Метка':('status',1),'БП':('target_hit',1),'Сбит с ног':('status',1),'Насыщение':('status',1),'Рассеивание':('status',1),'Чистые льды':('status',1)}
