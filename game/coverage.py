...
def inventory(text):
    ...
    def add(kind,title,start,end,body,data=None):
        ...
        if title=='Заряженные стрелы':handlers.append('game.charged_arrows')
        if title=='Ищущие стрелы':handlers.append('game.seeking_arrows')
        if title=='Передача истощения':handlers.append('game.prepared_attacks')
